...
public class CommandSet {
    ...
    public static final CommandSet.Entry SESSION_ID = CommandSetEntryBuilder.newInstance().opt("si")
            .longOpt("session-id").description("the session id of this session").hasArgs(true).numOfArgs(1)
            .argNames("session-id").jsCommand("setSessionId(sessionId)").commandClass(SetSessionCommand.class)
            .entry();

    public static final CommandSet.Entry SESSION_ID_FILE = CommandSetEntryBuilder.newInstance().opt("sif")
    ...
    public static final CommandSet.Entry LOGIN = CommandSetEntryBuilder.newInstance().opt("l")
            .longOpt("login").description("the login name to connect to REST server").hasArgs(true)
            .numOfArgs(1).argNames("login-name").jsCommand("login(login-name)")
            .commandClass(LoginCommand.class).entry();

    public static final CommandSet.Entry PASSWORD = CommandSetEntryBuilder.newInstance().opt("p")
            .longOpt("password").description("the password to connect to REST server").hasArgs(true)
            .numOfArgs(1).argNames("password").commandClass(SetPasswordCommand.class).entry();

    public static final CommandSet.Entry CREDENTIALS = CommandSetEntryBuilder.newInstance().opt("c")
            .longOpt("credentials").description("Path to the credential file").hasArgs(true).numOfArgs(1)
            .argNames("cred-path").jsCommand("loginwithcredentials(cred-path)")
            .commandClass(LoginWithCredentialsCommand.class).entry();

    public static final CommandSet.Entry CA_CERTS = CommandSetEntryBuilder.newInstance().opt("ca")
            .longOpt("cacerts").description("CA certificate store (JKS type) to verify peer against (SSL)")
            .hasArgs(true).numOfArgs(1).argNames("store-path").jsCommand("cacerts(store-path)")
            .commandClass(SetCaCertsCommand.class).entry();

    public static final CommandSet.Entry CA_CERTS_PASSWORD = CommandSetEntryBuilder.newInstance().opt("cap")
            .longOpt("cacertspass").description("Password for CA certificate store").hasArgs(true)
            .numOfArgs(1).argNames("store-password").jsCommand("cacertspass(cacerts-pass)")
            .commandClass(SetCaCertsPassCommand.class).entry();

    public static final CommandSet.Entry INSECURE = CommandSetEntryBuilder.newInstance().opt("k")
            .longOpt("insecure").description("Allow connections to SSL sites without certs verification")
            .commandClass(SetInsecureAccessCommand.class).entry();

    public static final CommandSet.Entry SILENT = CommandSetEntryBuilder.newInstance().opt("z")
            .longOpt("silent").description("Runs the command-line client in the silent mode.")
            .jsCommand("silent()").commandClass(SetSilentCommand.class).entry();

    public static final CommandSet.Entry PRINT = CommandSetEntryBuilder.newInstance().opt("pr")
            .longOpt("print").description("Print the session id").jsCommand("prints()")
            .commandClass(PrintSessionCommand.class).entry();

    public static final CommandSet.Entry OUTPUT = CommandSetEntryBuilder.newInstance().opt("o")
            .longOpt("output-file").description("Output the result of command execution to specified file")
            .hasArgs(true).numOfArgs(1).argNames("output-file").commandClass(OutputCommand.class).entry();
    ...
    public static final CommandSet.Entry SUBMIT = CommandSetEntryBuilder.newInstance().opt("s")
            .longOpt("submit").description("Submit the specified job-description (XML) file").hasArgs(true)
            .numOfArgs(1).hasOptionalArg(true).argNames("job-descriptor [var1=value1 var2=value2 ...]")
            .jsCommand("submit(job-descriptor,[var1=value1,var2=value2,...]")
            .commandClass(SubmitJobCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_START = CommandSetEntryBuilder.newInstance().opt("start")
            .longOpt("startscheduler").description("Start the Scheduler").jsCommand("start()")
            .commandClass(StartCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_STOP = CommandSetEntryBuilder.newInstance().opt("stop")
            .longOpt("stopscheduler").description("Stop the Scheduler").jsCommand("stop()")
            .commandClass(StopCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_PAUSE = CommandSetEntryBuilder.newInstance().opt("pause")
    ...
    public static final CommandSet.Entry SCHEDULER_RESUME = CommandSetEntryBuilder.newInstance().opt("resume")
            .longOpt("resumescheduler").description("Resume the Scheduler").jsCommand("resume()")
            .commandClass(ResumeCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_FREEZE = CommandSetEntryBuilder.newInstance().opt("freeze")
    ...
    public static final CommandSet.Entry SCHEDULER_KILL = CommandSetEntryBuilder.newInstance().opt("kill")
            .longOpt("killscheduler").description("Kill the Scheduler").jsCommand("kill()")
            .commandClass(KillCommand.class).entry();

    public static final CommandSet.Entry SCHEDULER_STATS = CommandSetEntryBuilder.newInstance().opt("sstats")
            .longOpt("schedulerstats").description("Retrieve current Scheduler statistics")
            .jsCommand("schedulerstats()").commandClass(SchedStatsCommand.class).entry();

    public static final CommandSet.Entry LINK_RM = CommandSetEntryBuilder.newInstance().opt("lrm")
            .longOpt("linkrm").description("Reconnect a Resource Manager to the Scheduler").hasArgs(true)
            .numOfArgs(1).argNames("rm-url").jsCommand("linkrm(rm_url)").commandClass(LinkRmCommand.class)
            .entry();

    public static final CommandSet.Entry JOB_LIST = CommandSetEntryBuilder.newInstance().opt("lj")
            .longOpt("listjobs")
    ...
    public static final CommandSet.Entry JOB_TASK_LIST_BY_TAG = CommandSetEntryBuilder.newInstance()
            .opt("ljt").longOpt("listjobtasks")
            .description("Retrieve the list of tasks names for a given job and a specified tag.")
            .hasArgs(true).numOfArgs(1).hasOptionalArg(true).argNames("jobId [tag offset limit]")
            .jsCommand("listtasks(job-id[,[tag|offset,limit]])").commandClass(ListJobTasksCommand.class)
            .entry();
    ...
    public static final CommandSet.Entry JS_LIST_LATEST = CommandSetEntryBuilder.newInstance()
            .description("Retrieves a list of the latest 'x' number of jobs.").jsCommand("listjobs(x)")
            .commandClass(ListJobCommand.class).entry();
    ...
    public static final CommandSet.Entry JOB_OUTPUT = CommandSetEntryBuilder.newInstance().opt("jo")
            .longOpt("joboutput")
            .description("Retrieve the output of specified job. This output can be filtered by a tag")
            .hasArgs(true).numOfArgs(1).hasOptionalArg(true).argNames("job-id [tag]")
            .jsCommand("joboutput(job-id[,tag])").commandClass(GetJobOutputCommand.class).entry();
    ...
    public static final CommandSet.Entry JOB_PRIORITY = CommandSetEntryBuilder.newInstance().opt("jp")
            ...
            .numOfArgs(2).argNames("job-id priority").jsCommand("jobpriority(job_id,priority)")
            .commandClass(ChangeJobPriorityCommand.class).entry();

    public static final CommandSet.Entry JOB_RESULT = CommandSetEntryBuilder.newInstance().opt("jr")
            .longOpt("jobresult")
            .description("Retrieve the result of specified job. This result can be filtered by a tag")
            .hasArgs(true).numOfArgs(1).hasOptionalArg(true).argNames("job-id [tag]")
            .jsCommand("jobresult(job-id[,tag])").commandClass(GetJobResultCommand.class).entry();

    public static final CommandSet.Entry JOB_STATE = CommandSetEntryBuilder.newInstance().opt("js")
            .longOpt("jobstate").description("Retrieve the current state of specified job").hasArgs(true)
            .numOfArgs(1).argNames("job-id").jsCommand("jobstate(job-id)")
            .commandClass(GetJobStateCommand.class).entry();

    public static final CommandSet.Entry JOB_PAUSE = CommandSetEntryBuilder.newInstance().opt("pj")
            .longOpt("pausejob").description("Pause the specified job (pause all non-running tasks)")
            .hasArgs(true).numOfArgs(1).argNames("job-id").jsCommand("pausejob(job-id)")
            .commandClass(PauseJobCommand.class).entry();

    public static final CommandSet.Entry JOB_RESUME = CommandSetEntryBuilder.newInstance().opt("rj")
            .longOpt("resumejob").description("Resume the specified job (restart all 'paused' tasks)")
            .hasArgs(true).numOfArgs(1).argNames("job-id").jsCommand("resumejob(job_id)")
            .commandClass(ResumeJobCommand.class).entry();

    public static final CommandSet.Entry JOB_KILL = CommandSetEntryBuilder.newInstance().opt("kj")
            .longOpt("killjob").description("Kill the specfied job").hasArgs(true).numOfArgs(1)
            .argNames("job-id").jsCommand("killjob(job-id)").commandClass(KillJobCommand.class).entry();
    ...
    public static final CommandSet.Entry TASKS_STATES = CommandSetEntryBuilder.newInstance().opt("ts")
            .longOpt("taskstates")
            .description(
                    "Retrieve the tasks states of a set of tasks belonging to a specified job and filtered by a tag")
            .hasArgs(true).numOfArgs(1).hasOptionalArg(true).argNames("job-id [tag offset limit]")
            .jsCommand("taskstates(job-id [,tag] [,offset,limit])").commandClass(ListTaskStatesCommand.class)
            .entry();
    ...
    public static final CommandSet.Entry TASK_PREEMPT = CommandSetEntryBuilder.newInstance().opt("pt")
            ...
            .jsCommand("preempttask(job-id,task-name,delay)").commandClass(PreemptTaskCommand.class).entry();
    ...
    public static final CommandSet.Entry TASK_RESTART = CommandSetEntryBuilder.newInstance().opt("rt")
            ...
            .jsCommand("restarttask(job-id,task-name)").commandClass(RestartTaskCommand.class).entry();
    ...
    public static final CommandSet.Entry LIVE_LOG = CommandSetEntryBuilder.newInstance().opt("ll")
            .longOpt("livelog").description("Retrieve the live output of specified job").hasArgs(true)
            .numOfArgs(1).argNames("job-id").jsCommand("livelog(job-id)").commandClass(LiveLogCommand.class)
            .entry();

    public static final CommandSet.Entry EVAL = CommandSetEntryBuilder.newInstance().opt("sf")
            .longOpt("script").description("Evaluate the specified JavaScript file").hasArgs(true)
            .numOfArgs(1).hasOptionalArg(true).argNames("script-path [param-1=value-1 param-2=value-2 ...]")
            .jsCommand("script(script-pathname,param1=value1,...)").commandClass(EvalScriptCommand.class)
            .entry();
    ...
    public static final CommandSet.Entry PUT_THIRD_PARTY_CREDENTIAL = CommandSetEntryBuilder.newInstance()
            ...
            .jsCommand("putcredential(key,value)").commandClass(PutThirdPartyCredentialCommand.class).entry();

    public static final CommandSet.Entry REMOVE_THIRD_PARTY_CREDENTIAL = CommandSetEntryBuilder.newInstance()
            .opt("rc").longOpt("remove-credential").numOfArgs(1).argNames("key")
            .description("Remove a third-party credential corresponding to the key from the Scheduler")
            .jsCommand("removecredential(key)").commandClass(RemoveThirdPartyCredentialCommand.class).entry();

    public static final CommandSet.Entry THIRD_PARTY_CREDENTIAL_KEY_SET = CommandSetEntryBuilder.newInstance()
            .opt("lc").longOpt("list-credentials")
    ...
    public static final CommandSet.Entry SCHEDULER_HELP = CommandSetEntryBuilder.newInstance().opt("sh")
    ...
    public static final CommandSet.Entry NODE_ADD = CommandSetEntryBuilder.newInstance().opt("a")
            .longOpt("addnodes").description("Add the specified nodes by their URLs").hasArgs(true)
            .numOfArgs(1).argNames("node-url").jsCommand("addnode(node-url[,node-source])")
            .commandClass(AddNodeCommand.class).entry();

    public static final CommandSet.Entry NS_CREATE = CommandSetEntryBuilder.newInstance().opt("cn")
            .longOpt("createns").description("Create new node source").hasArgs(true).numOfArgs(1)
            .argNames("node-source").jsCommand("createns(node-source)")
            .commandClass(CreateNodeSourceCommand.class).entry();

    public static final CommandSet.Entry NODE_REMOVE = CommandSetEntryBuilder.newInstance().opt("d")
            .longOpt("removenodes").description("Remove the specified node").hasArgs(true).numOfArgs(1)
            .argNames("node-url").jsCommand("removenode(node-url)").commandClass(RemoveNodeCommand.class)
            .entry();

    public static final CommandSet.Entry FORCE = CommandSetEntryBuilder.newInstance().opt("f")
            .longOpt("force").description("Do not wait for buys nodes to be freed before removal")
    ...
    public static final CommandSet.Entry INFRASTRUCTURE = CommandSetEntryBuilder.newInstance().opt("in")
            .longOpt("infrastructure").description("Specify an infrastructure for the node source")
            .hasArgs(true).numOfArgs(Option.UNLIMITED_VALUES).argNames("param1 param2 ...")
            .commandClass(SetInfrastructureCommand.class).entry();

    public static final CommandSet.Entry POLICY = CommandSetEntryBuilder.newInstance().opt("po")
            .longOpt("policy").description("Specify a policy for the node source").hasArgs(true)
            .numOfArgs(Option.UNLIMITED_VALUES).argNames("param1 param2 ...")
            .commandClass(SetPolicyCommand.class).entry();

    public static final CommandSet.Entry NODE_LIST = CommandSetEntryBuilder.newInstance().opt("ln")
            .longOpt("listnodes").description("List nodes handled by the Resource Manager")
            .jsCommand("listnodes([node-source])").commandClass(ListNodeCommand.class).entry();

    public static final CommandSet.Entry NS_LIST = CommandSetEntryBuilder.newInstance().opt("lns")
            .longOpt("listns").description("List node-sources handled by the Resource Manager")
            .jsCommand("listns()").commandClass(ListNodeSourceCommand.class).entry();

    public static final CommandSet.Entry NODE_LOCK = CommandSetEntryBuilder.newInstance().opt("lon")
            .longOpt("locknodes").description("Lock specified nodes").hasArgs(true)
            .numOfArgs(Option.UNLIMITED_VALUES).argNames("node1-url node2-url ...")
            .jsCommand("locknodes([node1-url,node2-url,...])").commandClass(LockNodeCommand.class).entry();

    public static final CommandSet.Entry NODE_INFO = CommandSetEntryBuilder.newInstance().opt("ni")
            .longOpt("nodeinfo").description("Retrieve info of specified node").hasArgs(true).numOfArgs(1)
            .argNames("node-url").commandClass(GetNodeInfoCommand.class).jsCommand("nodeinfo(node-url)")
            .entry();

    public static final CommandSet.Entry NS = CommandSetEntryBuilder.newInstance().opt("ns")
            .longOpt("nodesource").description("Specify node source name").hasArgs(true).numOfArgs(1)
            .argNames("node-source").commandClass(SetNodeSourceCommand.class).entry();

    public static final CommandSet.Entry NS_REMOVE = CommandSetEntryBuilder.newInstance().opt("r")
            .longOpt("removens").description("Remove specified node source").hasArgs(true).numOfArgs(1)
            .argNames("node-source").jsCommand("removens(node-source)")
            .commandClass(RemoveNodeSourceCommand.class).entry();

    public static final CommandSet.Entry TOPOLOGY = CommandSetEntryBuilder.newInstance().opt("t")
            .longOpt("topology").description("Retrieve node topology").jsCommand("topology()")
    ...
    public static final CommandSet.Entry NODE_UNLOCK = CommandSetEntryBuilder.newInstance().opt("ulon")
            .longOpt("unlocknodes").description("Unlock specified nodes").hasArgs(true)
            .numOfArgs(Option.UNLIMITED_VALUES).argNames("node1-url node2-url ...")
            .jsCommand("unlocknodes([node1-url,node2-url,...])").commandClass(UnlockNodeCommand.class)
            .entry();

    public static final CommandSet.Entry INFRASTRUCTURE_LIST = CommandSetEntryBuilder.newInstance().opt("li")
            .longOpt("listinfra").description("List supported infrastructure types")
    ...
    public static final CommandSet.Entry RM_HELP = CommandSetEntryBuilder.newInstance().opt("rh")
            .longOpt("rmhelp")
    ...
    public static final CommandSet.Entry SCHED_JS_HELP = CommandSetEntryBuilder.newInstance()
            .description("Help on Scheduler commands").jsCommand("schedulerhelp()")
    ...
    public static final CommandSet.Entry RM_JS_HELP = CommandSetEntryBuilder.newInstance()
            .description("Help on RM commands").jsCommand("rmhelp()").commandClass(RmJsHelpCommand.class)
            .entry();

    public static final CommandSet.Entry COMMON_HELP = CommandSetEntryBuilder.newInstance().opt("h")
            .longOpt("help").description("Prints the usage of REST command-line client for RM/Scheduler")
    ...
    public static final CommandSet.Entry COMMON_JS_HELP = CommandSetEntryBuilder.newInstance()
            .description("Interactive help").jsCommand("help()").commandClass(JsHelpCommand.class).entry();

    public static final CommandSet.Entry VERSION = CommandSetEntryBuilder.newInstance().opt("v")
            .longOpt("version").description("Display the version number").commandClass(VersionCommand.class)
            .jsCommand("version()").entry();

    /**
     * CommandSet.Entry objects which are common to both Scheduler and Resource
     * Manager CLIs
     */
    public static final CommandSet.Entry[] COMMON_COMMANDS = new CommandSet.Entry[] { URL, SESSION_ID,
            SESSION_ID_FILE, PASSWORD, CREDENTIALS, INSECURE, CA_CERTS, CA_CERTS_PASSWORD, EVAL, SILENT,
            PRINT, OUTPUT, COMMON_HELP, DEBUG, VERSION };

    /**
     * CommandSet.Entry objects which are specific to Scheduler CLI
     */
    public static final CommandSet.Entry[] SCHEDULER_ONLY = new CommandSet.Entry[] { SCHEDULER_START,
            SCHEDULER_STOP, SCHEDULER_PAUSE, SCHEDULER_RESUME, SCHEDULER_FREEZE, SCHEDULER_KILL, LINK_RM,
            SCHEDULER_STATS, JOB_LIST, SUBMIT, JOB_STATE, JOB_OUTPUT, JOB_RESULT, JOB_PRIORITY, JOB_PAUSE,
            JOB_RESUME, JOB_KILL, JOB_TASK_LIST_BY_TAG, JOB_REMOVE, TASK_RESTART, TASK_PREEMPT, TASK_OUTPUT,
    ...
    public static final CommandSet.Entry[] RM_ONLY = new CommandSet.Entry[] { LOGIN, NODE_ADD, NODE_LIST,
            NODE_INFO, NODE_LOCK, NODE_UNLOCK, NODE_REMOVE, NS_CREATE, NS_LIST, NS_REMOVE, NS, INFRASTRUCTURE,
            INFRASTRUCTURE_LIST, POLICY, POLICY_LIST, TOPOLOGY, FORCE, RM_STATS, RM_HELP };
        ...
        public static Command newCommandObject(Option opt, Class<?> commandClass) {
            try {
                ...
                throw new CLIException(REASON_INVALID_ARGUMENTS,
                    String.format("%s %s %s:%n%s %s.", "No suitable command found for", opt.getOpt(),
                            opt.getValuesList(),
